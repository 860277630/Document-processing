// Copyright (C) 2003-2022, Foxit Software Inc..
// All Rights Reserved.
//
// http://www.foxitsoftware.com
//
// The following code is copyrighted and contains proprietary information and trade secrets of Foxit Software Inc..
// You cannot distribute any part of Foxit PDF SDK to any third party or general public,
// unless there is a separate license agreement with Foxit Software Inc. which explicitly grants you such rights.
//
// This file contains an example to demonstrate how to use Foxit PDF SDK to flatten PDF pages.

import com.foxit.sdk.PDFException;
import com.foxit.sdk.common.Library;
import com.foxit.sdk.pdf.PDFDoc;
import com.foxit.sdk.pdf.PDFPage;

import java.io.File;

import static com.foxit.sdk.common.Constants.e_ErrSuccess;
import static com.foxit.sdk.pdf.PDFDoc.e_SaveFlagNormal;
import static com.foxit.sdk.pdf.PDFPage.*;

public class pdfflatten {
    private static String key = "";
    private static String sn = "tRs3tntQBkjXIQdh7XzekjZTJZ+lkTbQyBEVrQowtRotaU49uX2EdQ==";
    private static String output_path = "../output_files/pdfflatten/";
    private static String input_path = "../input_files/";

    // You can also use System.load("filename") instead. The filename argument must be an absolute path name.
    static {
        String os = System.getProperty("os.name").toLowerCase();
        String lib = "fsdk_java_";
        if (os.startsWith("win")) {
            lib += "win";
        } else if (os.startsWith("mac")) {
            lib += "mac";
        } else {
            lib += "linux";
      	}
        if (System.getProperty("sun.arch.data.model").equals("64")) {
            lib += "64";
        } else {
            lib += "32";
        }
        System.loadLibrary(lib);
    }

    private static void createResultFolder(String output_path) {
        File myPath = new File(output_path);
        if (!myPath.exists()) {
            myPath.mkdir();
        }
    }

    public static void main(String[] args) throws PDFException {
        createResultFolder(output_path);
        String input_file = input_path + "annotations.pdf";
        //Initialize library
        int error_code = Library.initialize(sn, key);
        if (error_code != e_ErrSuccess) {
            System.out.println("Library Initialize Error: " + error_code);
            return;
        }

        try {
            {
                // Load PDF document.
                PDFDoc doc = new PDFDoc(input_file);
                error_code = doc.load(null);
                if (error_code != e_ErrSuccess) {
                    System.out.println("The Doc " + input_file + " Error: " + error_code);
                    return;
                }
                // Get page count.
                int pageCount = doc.getPageCount();
                // Flatten PDF pages.
                for (int i = 0; i < pageCount; i++) {
                    PDFPage page = doc.getPage(i);
                    // Parse page before flattening the page.
                    page.startParse(e_ParsePageNormal, null, false);
                    page.flatten(true, e_FlattenAll);
                }

                String flatten_all = output_path + "annotations_flatten_all.pdf";
                doc.saveAs(flatten_all, e_SaveFlagNormal);

            }

            {
                // Flatten PDF with no annotation
                PDFDoc doc = new PDFDoc(input_file);
                error_code = doc.load(null);
                if (error_code != e_ErrSuccess) {
                    System.out.println("The Doc " + input_file + " Error: " + error_code);
                    return;
                }
                // Get page count.
                int pageCount = doc.getPageCount();
                // Flatten PDF pages.
                for (int i = 0; i < pageCount; i++) {
                    PDFPage page = doc.getPage(i);
                    // Parse page before flattening the page.
                    page.startParse(e_ParsePageNormal, null, false);
                    page.flatten(true, e_FlattenNoAnnot);
                }

                String flatten_noannot = output_path + "annotations_flatten_noannot.pdf";
                doc.saveAs(flatten_noannot, e_SaveFlagNormal);
            }

            {
                // Flatten PDF with no form control
                PDFDoc doc = new PDFDoc(input_file);
                error_code = doc.load(null);
                if (error_code != e_ErrSuccess) {
                    System.out.println(String.format("The Doc [%s] Error: %d\n", input_file, error_code));
                    return;
                }
                // Get page count.
                int pageCount = doc.getPageCount();
                // Flatten PDF pages.
                for (int i = 0; i < pageCount; i++) {
                    PDFPage page = doc.getPage(i);
                    // Parse page before flattening the page.
                    page.startParse(e_ParsePageNormal, null, false);
                    page.flatten(true, e_FlattenNoFormControl);
                }

                String flatten_noformcontrol = output_path + "annotations_flatten_noformcontrol.pdf";
                doc.saveAs(flatten_noformcontrol, e_SaveFlagNormal);
            }
            {
                // Flatten PDF with no form control and no annotation -- equals to nothing to be flattened.
                PDFDoc doc = new PDFDoc(input_file);
                error_code = doc.load(null);
                if (error_code != e_ErrSuccess) {

                    System.out.println(String.format("The Doc [%s] Error: %d\n", input_file, error_code));
                    return;
                }
                // Get page count.
                int pageCount = doc.getPageCount();
                // Flatten PDF pages.
                for (int i = 0; i < pageCount; i++) {
                    PDFPage page = doc.getPage(i);
                    // Parse page before flattening the page.
                    page.startParse(e_ParsePageNormal, null, false);
                    page.flatten(true, e_FlattenNoAnnot | e_FlattenNoFormControl);
                }

                String flatten_noannotnoform = output_path + "annotations_flatten_noannotnoform.pdf";
                doc.saveAs(flatten_noannotnoform, e_SaveFlagNormal);
                System.out.println("Flatten PDF");

            }

        } catch (Exception e) {

        }
        Library.release();
    }
}
