...
//verify paging seal signature in PDF document.
...
class SignatureCallbackImpl extends SignatureCallback {
	...
	public byte[] sign(byte[] digest, String cert_path, byte[] cert_password, int digest_algorithm, java.lang.Object client_data){
		try {
			...
			byte[] encryptStr = cerUtil.SignMsg(arrall, paging_seal_signature.input_path
	...
	public int verifySigState(byte[] var1, byte[] var2, Object var3) {
		...
		try {
			verify_state = cerUtil.VerifyMsg(var2, arrall,
					paging_seal_signature.input_path + "foxit.cer");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
...
public class paging_seal_signature {
	...
	public static String output_directory = output_path + "paging_seal_signature/";
 ...
 static {
	...
	static com.foxit.sdk.pdf.PagingSealSignature AddPagingSealSignature(PDFDoc pdf_doc,
			String sub_filter, boolean is_on_perforation) throws Exception {
		PDFPage pdf_page = pdf_doc.getPage(0);
		float page_height = pdf_page.getHeight();
		float page_width = pdf_page.getWidth();
		float sig_width =  page_width * (float)0.4;
		float sig_hegiht = page_height * (float)0.1;
		// Add new paging seal signature to document.
		Range range = new Range(1,pdf_doc.getPageCount(), Range.e_All);
		com.foxit.sdk.pdf.PagingSealSignature new_sig = pdf_doc.addPagingSealSignature(range, sig_width, sig_hegiht);
		if (new_sig.isEmpty()) {
			throw new Exception("Add signature failed!");
		}
		PagingSealConfig paging_seal_config = new PagingSealConfig(PagingSealConfig.e_PagingSealPositionLeft, (float)page_width/2, 0.5f, is_on_perforation);
		new_sig.setPagingSealConfig(paging_seal_config);
		boolean gaResult = new_sig.generateAppearance();
	...
	static void AdobePPKLitePagingSealSignature(PDFDoc pdf_doc, boolean use_default) throws Exception {
		...
		// Add a new paging seal signature to document.						
		boolean is_on_perforation = true;
		if (use_default) is_on_perforation = false;
		com.foxit.sdk.pdf.PagingSealSignature new_paging_seal_signature = AddPagingSealSignature(pdf_doc, sub_filter, is_on_perforation);		
		...
		boolean is_signed = new_paging_seal_signature.isSigned();
		int sig_state = new_paging_seal_signature.getState();
		...
		String signed_pdf_path = output_directory + "signed_middle_paging_seal_signature.pdf";
		if (use_default)
			signed_pdf_path = output_directory + "signed_left_paging_seal_signature_default_handler.pdf";
		...
		new_paging_seal_signature.startSign(cert_file_path, cert_file_password.getBytes(),
	...
	static void CheckPagingSealSignatureInfo(PDFDoc pdf_doc) throws PDFException {
		...
		for (int i = 0; i < sig_count; i++) {
			System.out.println(String.format("Signature index: %d\r\n", i));
		    com.foxit.sdk.pdf.Signature signature = pdf_doc.getSignature(i);
		    

		    if (signature.isEmpty()) continue;
			if (signature.getSignatureType() != com.foxit.sdk.pdf.Signature.e_SignatureTypePagingSeal) continue;
		    boolean is_signed = signature.isSigned();
		    System.out.println(String.format("Signed?:%s", is_signed ? "true": "false"));
		    int sig_state = signature.getState();
		    System.out.println(String.format("State:%s", TransformSignatureStateToString(sig_state)));
		    Widget widget_annot = signature.getControl(0).getWidget();
			System.out.println(String.format("Object number %d of related widget annotation's dictionary", widget_annot.getDict().getObjNum()));
			...
			System.out.println(String.format("Sub-filter:%s", sub_filter));
		    if (is_signed) {
				DateTime sign_time = signature.getSignTime();
				String time_str = DateTimeToString(sign_time);
			    System.out.println(String.format("Sign Time:%s", time_str));
		    }
		    String key_value;
			...
			System.out.println(String.format("Location:%s", key_value));
			...
			System.out.println(String.format("Contact Info:%s", key_value));
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < 2; i++) {		
			...
			// Check information of existed signature(s) in PDF file if there's any paging seal signature in the PDF file.
			CheckPagingSealSignatureInfo(pdf_doc);
			// Add paging seal signature, sign it and verify it with filter "Adobe.PPKLite" and different subfilter.
	    
			AdobePPKLitePagingSealSignature(pdf_doc, i > 0 ? true:false);
			String signed_pdf_path = output_directory + "signed_middle_paging_seal_signature.pdf";
			if (i > 0)
				signed_pdf_path = output_directory + "signed_left_paging_seal_signature_default_handler.pdf";
			PDFDoc pdf_doc1 = new PDFDoc(signed_pdf_path);
			pdf_doc1.startLoad(null, false, null);
